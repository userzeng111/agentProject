...
- 所有思考、总结，回复必须使用中文描述。
- 如果是在 Windows 下，控制台的编码默认为 utf-8，所有读取工具也需要用 utf-8。
- 项目问题（与当前仓库代码、配置、流程直接相关）需先记录问题并进入讨论，不得直接改代码。
- 非项目问题（通用问答、闲聊、与仓库无关事项）不进入 `worklog`，可直接处理。
- 优先使用 MCP 工具，为了保证信息安全，除非 MCP 满足不了，才使用其他的。
- 需求不明确有疑问的地方向我询问澄清，提供预定义选项与我交流讨论，给出选择选项，即将完成请求前必须向我询问，必须向我询问并得到可以完成任务/结束时，禁止主动结束对话/请求。
- 代码中的注释、报错信息这些也全部用中文。

- 复杂任务必须自动使用 Agent Teams 团队模式或者 subagent 模式工作，先分析好需要的角色，然后并行启动，当前任务结束后自动回收。
- Agent Teams 的角色选用模型在没有指定的情况下，默认为当前使用的模型，模型等级由你决定。
...
- 始终优先使用 `github` 来查看 GitHub 上的源码、文件等资料。
