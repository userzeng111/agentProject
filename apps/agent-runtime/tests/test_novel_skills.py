...
instances:
  - instance_id: douluo
    ...
    description: "术语流"
            ...
            self.assertEqual(context["canon_guidance"], "")
    ...
    def test_build_runtime_context_for_fanfic_uses_canon_guidance(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            workflow_root = root / "novel-workflow"
            workflow_root.mkdir(parents=True, exist_ok=True)
  ...
  - instance_id: douluo
    ...
    description: "原作设定"
            ...
            context = service.build_runtime_context(
                mode="fanfic",
                style_profile_id="douluo",
                custom_style="保持史莱克学院时间线",
            )

            self.assertIn("世界观", context["canon_guidance"])
            self.assertIn("保持史莱克学院时间线", context["canon_guidance"])
            self.assertEqual(context["style_guidance"], "保持史莱克学院时间线")
            self.assertEqual(context["active_instance_id"], "douluo")
