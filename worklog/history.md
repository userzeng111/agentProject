...
- 标题：api-full-test 全覆盖审查
  路径：`worklog/archive/技能治理/20260723-02-api-full-test-全覆盖审查.md`
  摘要：以 OpenAPI 清单和声明式计划取代固定端点脚本，支持接口漂移门禁与增量模块测试。

- 标题：项目验证技能通用性与兼容性完善
  路径：`worklog/archive/技能治理/20260723-01-项目验证技能通用性与兼容性完善.md`
  摘要：完善 API 全量与界面冒烟技能的只读模式、清理兜底、端口兼容和离线评测。

- 标题：章节审核卡片标题挤压修复
  路径：`worklog/archive/UI优化/20260721-03-章节审核卡片标题挤压修复.md`
  摘要：修复章节审核卡片中长摘要挤压标题为逐字换行的问题。

- 标题：章节审核页布局问题定位
  路径：`worklog/archive/UI优化/20260721-02-章节审核页布局问题定位.md`
  摘要：恢复章节审核工作台的左侧项目追踪布局和材料区可用高度。

- 标题：项目与设置侧栏导航纠正
  路径：`worklog/archive/UI优化/20260721-01-项目与设置侧栏导航纠正.md`
  摘要：统一项目追踪和设置导航的桌面左栏与窄屏替代交互。

- 标题：创作设定页体验优化方案
  路径：`worklog/archive/UI优化/20260720-03-创作设定页体验优化方案.md`
  摘要：完成三阶段创作工作台、准备度反馈与响应式创建体验。

- 标题：审核决策抽屉滚动修复
  路径：`worklog/archive/UI优化/20260720-02-审核决策抽屉滚动修复.md`
  摘要：修复审核决策抽屉内容滚动，保留固定标题与决策操作。

- 标题：项目审核工作台信息层级重构
  路径：`worklog/archive/UI优化/20260720-01-项目审核工作台信息层级重构.md`
  摘要：重构项目审核首屏层级、固定高优先级入口并释放材料空间。

- 标题：创建、归档与设置页可用性重构
  路径：`worklog/archive/UI优化/20260717-01-创建归档设置页可用性重构.md`
  摘要：完成创建、归档、设置工作台及设置子页面的可用性重构。

- 标题：页面内滚动与全局按钮固定及风格美化
  路径：`worklog/archive/UI优化/20260717-01-页面内滚动与全局按钮固定及风格美化.md`
  摘要：完成内容区滚动、固定主操作和统一工作台视觉体验改造。

- 标题：task_661e059fa3 章节分区修复失败
  路径：`worklog/archive/任务执行/20260716-05-task_661e059fa3-章节分区修复失败.md`
  摘要：修复章节分区解析、缺章恢复和窗口越界，保留已取消任务作为回归样本。

- 标题：task_6b53399dcd 修复与中篇 80 章真实验证
  路径：`worklog/archive/任务执行/20260716-04-task_6b53399dcd-中篇80章真实验证.md`
  摘要：修复批次章节生成，真实验证完成首个 20 章窗口；80 章全量慢测后续跟进。

- 标题：StoryEngine 严格 JSON 解析缺失
  路径：`worklog/archive/任务执行/20260716-03-StoryEngine严格JSON解析缺失.md`
  摘要：修复章节批次路径调用不存在严格 JSON 解析方法导致的运行失败。

- 标题：首页作品库删除后保持筛选状态
  路径：`worklog/archive/任务执行/20260716-02-首页作品库删除后保持筛选状态.md`
  摘要：作品库进入任务及删除返回均保留来源分类，避免回落默认分类。

- 标题：章节分页、删除与日志语义微调
  路径：`worklog/archive/任务执行/20260716-01-章节分页删除与日志语义微调.md`
  摘要：章节与历史日志分页、完成任务删除二次确认及重复摘要流清理已完成。

- 标题：task_dc6474ec0f：日志、章节进度与分批大纲
  路径：`worklog/archive/任务执行/20260715-01-task_dc6474ec0f-日志章节进度与分批大纲.md`
  摘要：完成实时日志累积、完整章节目录、历史日志存档与分批大纲硬门禁。

- 标题：task_ffcc008c19 误完成与正文缺失追踪
  路径：`worklog/archive/任务流转/20260715-01-task_ffcc008c19-误完成与正文缺失追踪.md`
  摘要：修复空大纲、空正文和幻觉章节补丁可绕过验证并误标 completed 的链路。

- 标题：任务级实时追踪日志缺失治理
  路径：`worklog/archive/日志治理/20260715-02-任务级实时追踪日志缺失治理.md`
  摘要：补齐工作流节点生命周期事件，修复跨线程 SSE 投递与思考片段丢失。

- 标题：大纲自动审核误恢复方案与审核卡住
  路径：`worklog/archive/任务恢复/20260715-01-大纲自动审核误恢复方案与审核卡住.md`
  摘要：自动审核保持 planning，运行期间禁用恢复方案，消除错误审核入口和并发冲突。
