...
- 标题：RAG 双模式同步设计与超时治理
  路径：`worklog/archive/性能优化/20260714-02-RAG双模式同步设计与超时治理.md`
  摘要：实现显式增量同步、全量二次确认与可轮询后台作业，消除同步请求超时与隐式全量构建。

- 标题：向量增量构建与生成聊天 UI 优化
  路径：`worklog/archive/性能优化/20260714-01-向量增量构建与生成聊天UI优化.md`
  摘要：完成来源级向量增量同步、移除结果页重复全文，并优化聊天 token 与思考过程响应式体验。
