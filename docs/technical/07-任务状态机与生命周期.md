...
sequenceDiagram
    ...
    U->>FE: 填写创作要求，选择已验证模型
    FE->>API: POST /tasks {model_id}
    API->>TS: create_task()
    TS-->>FE: 返回 task_id (状态: CREATED)

    U->>FE: 点击"开始生成"
    FE->>API: POST /tasks/{id}/run {model_id}
    API->>TS: run_task()
    ...
    U->>FE: 阅读大纲，点击"通过"
    FE->>API: POST /tasks/{id}/resume {approved, model_id}
    API->>TS: resume_task()
    TS->>TS: 标记 DRAFTING
...
任务在创建时已显式保存从当前供应商 `/models` 选择的模型。审核恢复或故障恢复可以携带新的 `model_id`；该值必须仍在当前供应商目录中且通过小说任务兼容性验证。未携带时只复用任务已保存的模型，不会回退到全局或默认模型。恢复前，系统会检查 checkpoint 里的 `input_payload.model_id` 是否与最终动作模型一致，不一致则打补丁：
...
- `_resolve_action_model_id`：用户可以为本次动作显式指定模型；未传时只复用任务创建时已显式保存的 `model_id`。两种来源都会校验模型仍在当前供应商目录中且可用于小说任务，绝不会使用系统默认模型。
...
- `model_id: str | None`：用户可在审核时传入新选择的模型 ID；未传时复用任务创建时已选模型。两者都必须来自当前供应商目录并通过小说任务兼容性验证。
...
`model_id` 为空时，恢复链路同样只复用任务创建时已显式保存的模型；若旧任务没有已选模型，接口返回 `400`，错误文案为：`任务没有已选模型，请在本次动作中显式选择当前供应商返回的模型。`。历史调用传入的 `default_model_id` 只会在入站时映射为显式 `model_id`，响应与新日志不会再写出该字段；恢复已有 checkpoint 时还会重新校验其中实际参与自动审核的模型。它不会读取全局或默认模型配置。
