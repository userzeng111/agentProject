...
4. **localStorage 足够**：只有"聊天历史"和"模型选择"需要跨页面持久化
...
graph LR
    ...
    E[动作模型选择] -->|localStorage| F[novel-agent:action-model-id]
```
...
sequenceDiagram
    ...
    User->>FE: 点击"开始执行"
    FE->>API: POST /api/tasks/{id}/run (runTask)
    ...
    User->>FE: 阅读大纲，填写审核意见
    User->>FE: 点击"通过"
    FE->>API: POST /api/tasks/{id}/resume (resumeTask, approved=true)
    ...
    alt 任务失败或取消
        ...
        FE->>FE: 显示恢复方案
        User->>FE: 点击"恢复任务"
        FE->>API: POST /api/tasks/{id}/recover (recoverTask)
        API-->>FE: TaskRecord { status: "planning" }
    end
```
